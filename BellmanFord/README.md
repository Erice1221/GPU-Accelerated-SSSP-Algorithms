...
2. Run with `./parallel -f PATH_TO_FILE` (example: ./parallel -f ./graph/test.txt)
...
## Input Graphs 
/graphs/ includes `make_test_graphs.py` which generates graphs that can be used on both versions (all *.txt files a run with checker.sh).
